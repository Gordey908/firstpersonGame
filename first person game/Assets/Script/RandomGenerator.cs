...
public class RandomGenerator : MonoBehaviour
{
    ...
    private int baseHeight = 2, maxBlockCountY = 10, chunkSize = 16, perlinNoiseSensitivity = 25, chunkCount = 4;
...
private void CreateChunk(int chunkNumX, int chunkNumZ)
    {
        ...
        for (int x = chunkNumX * chunkSize; x < chunkNumX * chunkSize + chunkSize; x++)
        {
            ...
            {
                float xSample = seedX + (float)x / perlinNoiseSensitivity;
                float ySample = seedY + (float)z / perlinNoiseSensitivity;
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    if (y == height - 1)
                    {
                        obj = Instantiate(grassPrefab, new Vector3(x, y, z), Quaternion.identity, chunk.transform);
